...
export function ProjectCard({ project, houseTypeCount }: { project: Project; houseTypeCount: number }) {
  ...
  return (
    // Link stays the outer element so the whole card is one focus/click target;
    // Card is composed inside it purely for the visual chrome (bg/border/radius).
    <Link href={`/projects/${project.id}`} className="dash__project">
      <Card padded={false} className="dash__project-card">
